...
string online(unsigned int server_id) {
	//check that there is such a user_id in the map!!!
	return ONLINE + serverNames[server_id] + '#' + to_string(server_id);
}

string offline(unsigned int server_id) {
	//check that there is such a user_id in the map!!!
	return OFFLINE + serverNames[server_id] + '#' + to_string(server_id);
}
...
int main() {
	...
	system("title WebSockets_Server-v1.6.31");
	...
	uWS::App(). //create a Server without encryptio
		...
		.sendPingsAutomatically = true,
			...
			.open = [&last_server_id](auto* ws) {
				...
				ws->publish(BROADCAST_CHANNEL, online(serverData->server_id));
			...
			.close = [](auto* ws, int code, string_view message) {
				PerSocketData* serverData = (PerSocketData*)ws->getUserData();
				ws->publish(BROADCAST_CHANNEL, offline(serverData->server_id));
				printf("\nServe %s#%s disconnected\n", serverData->serverName.c_str(), to_string(serverData->server_id).c_str());
